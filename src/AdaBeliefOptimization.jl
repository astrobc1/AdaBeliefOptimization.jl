...
Minimize an objective function `loss` with corresponding gradient `grad` using the AdaBeliefOptimization algorithm. Additional parameters can be passed to `loss` via the `extra` kwarg. The usual AdaBeliefOptimization hyperparameters `β1, β2, α, and ϵ` are tuneable kwargs. 
Returns a `NamedTuple` with fields `pbest, fbest, iterations`.
"""
function optimize(loss, grad, p0::AbstractArray{<:Real}, extra=nothing; miniters::Int=0, maxiters::Int, β1::Real=0.99, β2::Real=0.999, α::Real=0.001, ϵ::Real=1E-8, ftol::Real=1E-6, verbose::Bool=false)
    m0 = zero.(similar(p0))
    s0 = zero.(similar(p0))
    state = AdaBeliefState(β1, β2, α, ϵ, 0, deepcopy(p0), m0, s0, loss_wrapper(loss, p0, extra), similar(p0))
    while true
        f = state.f
        step!(state, loss, grad, extra)
        fnew = state.f
        if verbose
...
function step!(state::AdaBeliefState, loss, grad, extra)
    state.t += 1
